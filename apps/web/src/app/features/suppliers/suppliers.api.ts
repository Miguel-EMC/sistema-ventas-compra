...
export class SuppliersApiService {
  ...
  async list(search = ''): Promise<BusinessPartner[]> {
    ...
    const url = queryString === '' ? `${API_BASE_URL}/suppliers` : `${API_BASE_URL}/suppliers?${queryString}`;
  ...
  async create(payload: BusinessPartnerPayload): Promise<BusinessPartner> {
    const response = await firstValueFrom(
      this.http.post<ApiResourceResponse<BusinessPartner>>(`${API_BASE_URL}/suppliers`, payload),
    );
  ...
  async delete(id: number): Promise<void> {
    await firstValueFrom(this.http.delete(`${API_BASE_URL}/suppliers/${id}`));
  }
}
